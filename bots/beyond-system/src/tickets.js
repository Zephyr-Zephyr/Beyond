// Beyond System — ticket create/close/reopen/delete logic.
...
export async function createTicket(interaction, categoryName) {
  ...
  const existing = interaction.guild.channels.cache.find(
    (c) => c.topic === `beyond-ticket:${interaction.user.id}`
  );
  if (existing) {
  ...
  const channel = await interaction.guild.channels.create({
    ...
    topic: `beyond-ticket:${interaction.user.id}`,
  ...
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId("beyond_ticket_close").setLabel("Close").setEmoji("🔒").setStyle(ButtonStyle.Secondary)
  );
...
export async function closeTicket(interaction) {
  ...
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId("beyond_ticket_reopen").setLabel("Reopen").setEmoji("🔓").setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId("beyond_ticket_delete").setLabel("Delete").setEmoji("🗑️").setStyle(ButtonStyle.Danger)
  );
